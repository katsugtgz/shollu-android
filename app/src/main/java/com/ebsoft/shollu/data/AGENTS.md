...
- `PrayerRepository` cache is `ConcurrentHashMap<PrayerCalculationKey, PrayerTimes>` guarded by `cacheLock`; key = date + full city (lat/lon/elev/tz) + method + juristic + ihtiyat + offsets map. Any new calc input MUST join the key or stale results leak. Cap is `MAX_CALCULATION_CACHE` (400): over cap, clear then re-put the just-computed entry — eviction atomic with insert under `cacheLock`. `clearCache()` takes the same lock (tests).
