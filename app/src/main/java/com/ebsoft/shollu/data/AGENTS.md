...
- `PrayerRepository` cache is access-order `LinkedHashMap<PrayerCalculationKey, PrayerTimes>` guarded by `cacheLock` (not concurrent; mutate only under the lock); key = date + full city (lat/lon/elev/tz) + method + juristic + ihtiyat + offsets map. Any new calc input MUST join the key or stale results leak. Cap is `MAX_CALCULATION_CACHE` (400): `removeEldestEntry` drops the least-recently-accessed key on insert over cap — never wipe the whole map. Hits go through `get` so they refresh LRU order. `clearCache()` takes the same lock (tests).
